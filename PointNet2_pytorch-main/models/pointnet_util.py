...
def square_distance(src, dst):
    """
    Calculate Euclid distance between each two points. #计算两组点云每两点之间的欧氏距离的平方
...
def index_points(points, idx):
    ...
    new_points = points[batch_indices, idx, :] #points:[B, N, 3] 先进行Batch内索引，剩下的是[N, 3]，然后对每个点云进行[idx, :]操作
    return new_points
...
def farthest_point_sample(xyz, npoint): #采样点之间距离足够远
    ...
    farthest = torch.randint(0, N, (B,), dtype=torch.long).to(device) #随机初始化最远点的索引，随机数范围是从0-N，一共是B个，维度是1*B，保证每个B都有一个最远点
    ...
    for i in range(npoint):#寻找并选取空间中每个点距离多个采样点的最短距离，并存储在dist
        ...
        centroid = xyz[batch_indices, farthest, :].view(B, 1, 3) #取中心坐标也是farthest点
        dist = torch.sum((xyz - centroid) ** 2, -1) #求所有点到farthest点的欧式距离和
        mask = dist < distance
        distance[mask] = dist[mask]
        #点和点的距离对应的索引相同
        farthest = torch.max(distance, -1)[1] #返回最大距离的点，意思是最大距离的点对应的索引
    return centroids
...
def query_ball_point(radius, nsample, xyz, new_xyz): #寻找球半径里面的点，从S个球内采样nsample个点
    ...
    Input:
        ...
        new_xyz: query points, [B, S, 3]，S个球形领域中心点，由farthestpoint组成 前边fps得到
    Return:
        group_idx: grouped points index, [B, S, nsample]，输出球形领域采样点索引
    """
    device = xyz.device
    B, N, C = xyz.shape #原始点云的BNC
    _, S, _ = new_xyz.shape #由FPS得出的S，例如一共有S个球
    #获取点云的各个点的序列位置
...
#         """
#         Input:
#             xyz: input points position data, [B, 3, N] #输入索引
#             points: input points data, [B, D, N] #输入除坐标外的特征
#         Return:
#             new_xyz: sampled points position data, [B, 3, S] #每个邻域的索引：  B个batch 每个batch S个中心点  每个中心点3维特征
#             new_points_concat: sample points feature data, [B, D', S] #每个邻域的特征：  B个batch 每个batch S个中心点  每个中心点D'维特征
#         """
...
class PointNetSetAbstraction(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        new_points = torch.flatten(new_points, start_dim=2)
            #flatten
